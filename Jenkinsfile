pipeline {
    ...
    stages {
        stage('Git Checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/Mariam-Ghamgui/blog-app-CICD-EKS.git'
            }
        }
        stage('Compile') {
            steps {
        ...
        stage('SonarQube Analysis') {
            steps {
                withSonarQubeEnv('sonarqubeServer') {
                    sh '''$SCANNER_HOME/bin/sonar-scanner -Dsonar.projectName=blog-app-CICD-EKS -Dsonar.projectKey=blog-app-CICD-EKS \
        ...
        stage('Docker Build & Tag') {
            steps {
                ...
                sh "docker build -t mariamghamgui/blog-app ."
                }
                }
            }
        }
        stage('Trivy Image Scan') {
            steps {
                sh "trivy image --format table -o image.html mariamghamgui/blog-app:latest"
            }
        }
        stage('Docker Push Image') {
            steps {
                script{
                withDockerRegistry(credentialsId: 'dockerhub-cred', url: 'https://index.docker.io/v1/') {
                    sh "docker push mariamghamgui/blog-app"
                }
                }
            }
        }
